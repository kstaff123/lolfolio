...
export const computeMatchData = async (matchId, puuid) => {
  ...
  if (participantIndex === -1)
    throw new Error(`PUUID ${puuid} not found in participants`);
  ...
  const rune1 =
    participantData?.perks?.styles?.[0]?.selections?.[0]?.perk || null;
  ...
  const runeTreeIcons = [
    {
      id: 8000,
      link: "https://ddragon.leagueoflegends.com/cdn/img/perk-images/Styles/7201_Precision.png",
    },
    {
      id: 8100,
      link: "https://ddragon.leagueoflegends.com/cdn/img/perk-images/Styles/7200_Domination.png",
    },
    {
      id: 8200,
      link: "https://ddragon.leagueoflegends.com/cdn/img/perk-images/Styles/7202_Sorcery.png",
    },
    {
      id: 8300,
      link: "https://ddragon.leagueoflegends.com/cdn/img/perk-images/Styles/7203_Whimsy.png",
    },
    {
      id: 8400,
      link: "https://ddragon.leagueoflegends.com/cdn/img/perk-images/Styles/7204_Resolve.png",
    },
  ];

  const rune1Link = `https://ddragon.leagueoflegends.com/cdn/img/perk-images/Styles/${tree1}/${rune1Key}/${
    rune1Key == "LethalTempo" ? "LethalTempoTemp" : rune1Key
  }.png`;
...
function MatchType({ gameMode, gameDuration, win, date, isRanked }) {
  ...
  return (
    <div className="flex flex-col items-center flex-shrink-0 justify-center text-center">
      ...
      <p className="sm:text-xs font-reg text-2xs max-[640px]:leading-[12px] text-[#cddcfe]">
        {date}
      </p>
      <div
        className={`flex items-center space-x-2 ${
          isRanked ? `hidden` : "visible"
        } `}
      >
        <svg
          width="9"
        ...
        >
          <path
            d="M4.5 0L8.39711 4.5H0.602886L4.5 0Z"
            fill={win ? "#6C7FFF" : "#FF6C6C"}
          />
        </svg>
        <h1 className="font-semibold sm:text-lg max-[640px]:leading-[12px] text-2xs">
          {!remake ? Math.floor(Math.random() * 9) + 20 : 0} LP
        </h1>
      </div>
      <div className="flex">
        <h2
          className={`pr-1 font-bold sm:text-xs text-2xs max-[640px]:leading-[12px] ${
            win ? "text-[#6C7FFF]" : "text-[#FF6C6C]"
          } `}
        >
          {remake ? "REMAKE" : result}
        </h2>
        <p className="font-reg sm:text-xs text-2xs max-[640px]:leading-[12px] text-[#cddcfe]">
          {duration}
        </p>
      </div>
    </div>
    ...
    <div className="flex items-center flex-shrink-0 relative">
      <div className="relative">
        ...
        <div className="bg-neutral-800 sm:w-4 sm:h-4 w-3 h-3 text-center rounded-bl-md flex items-center justify-center absolute bottom-0 left-0">
          <p className="sm:text-[10px] text-[8px] text-white leading-none">
            {champLevel}
          </p>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col items-center flex-shrink-0 ">
      ...
      <div className="flex  ">
        ...
        <p className="font-reg sm:text-xs   text-2xs leading-[16px]  text-[#cddcfe] ">
          KDA
        </p>
      </div>
      <div className="flex items-center  leading-[16px] text-[#cddcfe]">
        <p className="font-reg sm:text-xs  text-2xs leading-[16px]">
          {totalMinionsKilled + neutralMinionsKilled} CS ({csPerMinute})
        </p>
      </div>
      <div className="flex">
        <p className="font-reg sm:text-xs text-2xs leading-[16px] text-[#cddcfe]">
...
function MatchItems({ items = [], win }) {
  ...
  const gridItems = [
    ...regularItems,
    ...Array(6 - regularItems.length).fill(0),
  ];
    ...
    <div className="flex flex-shrink-0">
      <div className="grid grid-cols-3 gap-1">
        {gridItems.map((item, index) => (
          <div
            key={index}
            className={`sm:w-6 sm:h-6 w-4  flex items-center justify-center rounded-md ${
              win ? "bg-[#23366b]" : "bg-[#53263e]"
            }`}
          >
            {item !== 0 && (
              <img
      ...
      <div className="sm:w-6 sm:h-6 w-4 bg-[#32274d] flex items-center justify-center rounded-md ml-1 h-fit">
        <img
